...
void check(string&a, string&b)
{
    ...
    //we didn't find any a[i]>b[i], xxa, xx
    if(sz(a)>sz(b))
    {
        a = a.substr(0, sz(b));
        return;
...
#endif
        // GrandMaster Sivav
        // Very Good Question, had to think for simpler implementation.
        // Funny that I didn't do the implementation which I got to mind at first.
        // by deleting some suffix we can only make a string lexicographically smaller (this was the key).
        // Found Solution very hard to believe, that it is correct
	int n;  cin>>n;
        vector<string> ar(n);
        for(auto&i: ar)
            cin>>i;
        ...
        for(int i = n-1; i >=1; i--)
        {
            //ar[i] < ar[i-1], so delete some suffix of ar[i-1];
            check(ar[i-1], ar[i]);
        }
        for(auto&i: ar)
            cout << i << ln;
