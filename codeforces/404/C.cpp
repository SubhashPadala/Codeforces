...
int32_t main()
{
	...
	int n; cin>>n>>k;
        ...
        //d[i]'s will be connected to d[i+1]'s and d[i]'s
        //pakka with d[i-1] to have connectivity.
        bool connected = true;
        if(d[0].size()!=1 || sz(d[1])>k)
            connected = false;
        dbg(connected);
        for(int i = 1; i < n; i++)
        {
            ...
            if(i < n and d[i+1].size()>(k-1)*d[i].size())
                connected = false;
        }
        dbg(d);
        dbg(connected);
