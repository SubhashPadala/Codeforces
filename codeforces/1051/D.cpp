...
#endif
	int n, k; cin>>n>>k;
        vector<vector<vector<int>>> dp(n, vector<vector<int>>(k+1+4, vector<int>(4)));
        dp[0][1][0] = 1;
        dp[0][1][3] = 1;
        dp[0][2][2] = 1;
        dp[0][2][1] = 1;
        //dp[i][k][j] is cnt of having i columns, k components and j as ending point
        vector<vector<int>> increase(4, vi(4));
        for(int i = 0; i < 4; i++)
        {
            for(int j = 0; j < 4; j++)
            {
                ...
                if((i&2)!=(j&2))
                    increase[i][j]++;
                if((j&1)*2==(j&2) and (increase[i][j]!=0))
                    increase[i][j]--;
            }
        }
        for(int i = 1; i < n; i++)
        {
            for(int j = 0; j < k+1; j++)
            {
                for(int l = 0; l < 4; l++)
                {
                    ...
                    {
                        if(j-increase[prev][l]>=0)
                            dp[i][j][l] += dp[i-1][j-increase[prev][l]][prev];
                        dp[i][j][l]%=MOD2;
                    }
                }
            }
